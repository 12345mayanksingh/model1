...
#now we have obtained a good accuracy score so no need of hyperparameter tuning.
...
#THE SECOND QUESTION IS A EXAMPLE OF UNSUPERVISED MACHINE LEARNING ALGORITHM BECAUSE ONLY FEATURES ARE GIVEN,THERE IS NO LABELS.
...
#now we replace the gender columns and smoker column from object datatype to int data type
...
#now we will drop the region column because insurance didnt vary region wise
...
#after seeing this above table there might be some skewness in 'charges' and outliers are also present in this.But now we plot distplot
...
#after seeing the graph there is some skewness and outliers is there now will plot box plot to check for outliers
...
#Now skewness has been treated  now we plot box plot
...
for colom in df:
    if plotnumbers<=8:
        ax=plt.subplot(3,3,plotnumbers)
        sns.boxplot(df[colom])
        plt.xlabel(colom,fontsize=15)
    plotnumbers+=1
plt.show() 
...
index_b=np.where(df['bmi']>bmi_high)
index_b
...
index_ch=np.where(df['charges']>charges_high)
index_ch
...
#now all the outliers are treated now we will move further
...
scalar=StandardScaler()
x_scale=scalar.fit_transform(df)
x_scale
...
#By varying the values of clusters the final results change,this is the best result we are getting at cluster 2.
